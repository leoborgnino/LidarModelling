...
# Funcin para crear una pared (plano extruido)
...
# Funcin para crear y asignar un material
...
room_width = 4    # Ancho de la habitación (5 metros)
room_large = 3.46
room_height = 2.12  # Altura de las paredes (2.5 metros)
...
#Chapa de metal
bpy.ops.mesh.primitive_cube_add(size=1, enter_editmode=False, align='WORLD', location=( 1.9 ,0.095 , 0.07))  # Centro la caja a mitad de su altura
metal = bpy.context.object
metal.scale = (0.02/2, 0.99/2 , 0.74/2)  # Escala en metros: 33 cm largo, 23 cm ancho, 23 cm alto
metal.rotation_euler = (0, 0, 0)  # Rotar 90 grados sobre el eje X
bpy.ops.object.transform_apply(scale=True)
metal.name = "Metal"
...
bpy.ops.wm.save_as_mainfile(filepath="models/empty_room2.blend")
